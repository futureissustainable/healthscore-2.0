  ...
  return (
    <header className={`absolute top-0 left-0 right-0 z-20 ${className}`}>
      <nav className="container mx-auto max-w-7xl px-6 py-6 flex justify-between items-center">
        ...
        <div className="hidden sm:flex items-center gap-grid-6">
          <button
            onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
            className="text-p-sm text-slate-900 font-medium hover:text-slate-700 transition-colors relative"
          >
            Score
          ...
          <button
            ...
            className="text-p-sm text-slate-600 hover:text-slate-900 transition-colors"
          >
            Discover
          </button>
          <button
